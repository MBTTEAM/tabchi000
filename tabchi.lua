...
function process(msg)
  ...
  do
    ...
    if msg.text:match("^[!/#]block") and is_sudo(msg) and #matches == 2 then
      ...
      return "کاربر با موفقیت بلاک شد  😀"
    end
  end
    ...
    do
      ...
      local inline = function(arg, data)
        if data.results_ and data.results_[0] then
          ...
          local text = [[
پنل آمار ربات تبچی شما 😎 :

تعداد Pv تبچی شما 😐 : ]] .. pvs .. [[
          
تعداد گروه های معمولی شما : ]] .. gps .. [[
          
تعداد سوپر گروه های شما : ]] .. sgps .. [[
          
تعداد لینک های ذخیره شده گروه ها : ]] .. links
  ...
  do
    ...
    if msg.text:match("^[!/#]addsudo") and is_full_sudo(msg) and #matches == 2 then
      local text = matches[2] .. " ✅ این آقا با موفقیت به لیست ادمین ها اضافه شد "
  ...
  do
    ...
    if msg.text:match("^[!/#]remsudo") and is_full_sudo(msg) and #matches == 2 then
      local text = matches[2] .. " این آقا با موفقیت از لیست ادمین های تبچی حذف شد 👌🏻"
  ...
  do
    ...
    if msg.text:match("^[!/#]addedmsg") and is_sudo(msg) and #matches == 2 then
      if matches[2] == "on" then
        ...
        return "پیام ادد شدن مخاطب ها با موفقیت فعال شد 👌🏻"
      elseif matches[2] == "off" then
        redis:del("tabchi:" .. tabchi_id .. ":addedmsg")
        return "پیام ادد شدنمخاطب ها با موفقیت غیرفعال شد 👌🏻"
      end
    end
  end
  do
    ...
    if msg.text:match("^[!/#]markread") and is_sudo(msg) and #matches == 2 then
      if matches[2] == "on" then
        ...
        return "خواندن پیام ها توسط تبچی باموفقیت فعال شد 👌🏻"
      elseif matches[2] == "off" then
        redis:del("tabchi:" .. tabchi_id .. ":markread")
        return "خواندن پیام ها توسط تبچی شما باموفقیت غیرفعال شد 👌🏻"
      end
    end
  end
  do
    ...
    if msg.text:match("^[!/#]setaddedmsg") and is_sudo(msg) and #matches == 2 then
      ...
      return [[
پیام اد شدن مخاطب باموفقیت ذخیره شد....
_پیام_ :
]] .. matches[2]
    end
  end
  ...
  if msg.text:match("^[!/#]fwd all$") and msg.reply_to_message_id_ and is_sudo(msg) then
    ...
    return "پیام شما باموفقیت ارسال شد ❤️"
  end
  if msg.text:match("^[!/#]fwd gps$") and msg.reply_to_message_id_ and is_sudo(msg) then
    ...
    return "پیام شما به همه گروه ها ارسال شد ❤️"
  end
  if msg.text:match("^[!/#]fwd sgps$") and msg.reply_to_message_id_ and is_sudo(msg) then
    ...
    return "پیام شما به همه سوپر گروه ها ارسال شد ❤️"
  end
  ...
  if msg.text:match("^[!/#]fwd users$") and msg.reply_to_message_id_ and is_sudo(msg) then
    ...
    return "پیام شما به همه کاربران ارسال شد ❤️"
  end
  do
    local matches = {
      msg.text:match("[!/#](lua) (.*)")
